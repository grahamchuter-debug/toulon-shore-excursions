...
interface SignatureExperienceCalloutProps {
  context?: "cassis" | "toulon" | "general";
  compact?: boolean;
}

const CONTEXT_COPY = {
  cassis: "Considering Cassis? Our future Signature Provençal Toulon Discovery is being designed as a carefully paced small-group coastal day.",
  toulon: "Exploring Toulon? Our future Wow Collection will add distinctive small-group Provence routes developed around cruise timings.",
  general: "Our future flagship Signature Provençal Toulon Discovery is a proposed small-group experience. It is still in preparation.",
};

export function SignatureExperienceCallout({ context = "general", compact = false }: SignatureExperienceCalloutProps) {
  const image = subjectImages.harbour;
  ...
  if (compact) {
    return (
      <div className="card-signature mt-10">
        ...
        <h2 className="mt-4 font-display text-xl font-bold text-gray-900">{signatureToulonExperience.title}</h2>
