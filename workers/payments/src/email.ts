...
export function formatEmailFrom(env: PaymentsEnv): string {
  ...
  const name = (env.EMAIL_FROM_NAME ?? "Toulon Shore Excursions").trim();
...
export function buildInternalBookingEmail(
  ...
  const subject = `New paid Toulon booking — ${ref} — ${date}`;
  const adminUrl = `https://toulonshoreexcursions.com/admin/booking/${encodeURIComponent(ref)}`;
